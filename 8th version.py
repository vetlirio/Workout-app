...
def edit_selected_log(exercise):
    ...
    if not selection:
        return
    index = selection[0]
    entry = log_listbox.get(index)
    entry_entry.delete(0, tk.END)
    entry_entry.insert(0, entry)  # Load the selected entry for editing

    # When saving, replace the selected entry
    save_button.config(command=lambda: save_edited_log(exercise, index))

def save_edited_log(exercise, index):
    new_entry = entry_entry.get()
    filename = f"{exercise}.txt"
...
# Remove log button
remove_log_button = tk.Button(log_frame, text="Remove Log Entry", command=lambda: remove_log_entry(exercises[0]))
remove_log_button.pack(pady=5)

# Make log lines clickable for editing
log_listbox.bind("<ButtonRelease-1>", lambda event: edit_selected_log(exercises[0]))  # Pass the exercise name to function
